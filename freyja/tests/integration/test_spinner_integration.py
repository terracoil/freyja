...
from freyja.utils.spinner import CommandContext
...
class TestSpinnerIntegration:
    ...
    def test_command_context_extraction_simple(self):
        ...
        with patch.object(sys, 'argv', ['test_cli', '--global-param', 'test_global', '--debug', 
                                       'test-method', 'required_value', '--optional-param', 'test_optional']):
            result = cli.run()
    ...
    def test_command_context_extraction_hierarchical(self):
        ...
        with patch.object(sys, 'argv', ['data_cli', '--config', 'prod.json', '--verbose',
                                       'operations', '--workspace', '/tmp/data', '--backup',
                                       'process-file', 'data.csv', '--format-type', 'json']):
            result = cli.run()
    ...
    def test_completion_mode_bypasses_spinner(self):
        ...
        with patch.dict('os.environ', {'_FREYJA_COMPLETE': 'bash'}), \
    ...
    def test_sub_global_arguments_integration(self):
        ...
        with patch.object(sys, 'argv', ['proj_cli', '--config', 'prod.json',
                                       'file-ops', '--workspace', '/tmp/files', '--backup',
                                       'process', 'data.txt', '--format-type', 'csv']):
            result = cli.run()
