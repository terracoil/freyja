"""Tests for mod_example functionality."""
import subprocess
import sys
...
class TestModuleExample:
  """Test cases for the mod_example file."""

  def test_examples_help(self):
    """Test that mod_example shows help without errors."""
    examples_path = Path(__file__).parent.parent.parent / "examples" / "mod_example"
    result = subprocess.run(
      [str(examples_path), "--help"],
      capture_output=True,
      text=True,
      timeout=10
    )
  ...
  def test_examples_generate_report_command(self):
    """Test the generate-report command in mod_example."""
    examples_path = Path(__file__).parent.parent.parent / "examples" / "mod_example"
    ...
    try:
      result = subprocess.run(
        [str(examples_path), "generate-report", "--data-file", str(test_file)],
        capture_output=True,
        text=True,
        timeout=10
      )
  ...
  def test_examples_calculate_statistics_help(self):
    """Test the calculate-statistics command help in mod_example."""
    examples_path = Path(__file__).parent.parent.parent / "examples" / "mod_example"
    result = subprocess.run(
      [str(examples_path), "calculate-statistics", "--help"],
      capture_output=True,
      text=True,
      timeout=10
    )
  ...
  def test_examples_verify_file_hash_help(self):
    """Test the verify-file-hash command help in mod_example."""
    examples_path = Path(__file__).parent.parent.parent / "examples" / "mod_example"
    result = subprocess.run(
      [str(examples_path), "verify-file-hash", "--help"],
      capture_output=True,
      text=True,
      timeout=10
    )
...
class TestClassExample:
  ...
  def test_class_example_help(self):
    ...
    examples_path = Path(__file__).parent.parent.parent / "examples" / "cls_example"
    result = subprocess.run(
      [str(examples_path), "--help"],
      capture_output=True,
      text=True,
  ...
  def test_class_example_process_file(self):
    ...
    examples_path = Path(__file__).parent.parent.parent / "examples" / "cls_example"
    result = subprocess.run(
      [str(examples_path), "file-operations", "process-single", "--input-file", "test.txt"],
      capture_output=True,
      text=True,
      timeout=10
    )
  ...
  def test_class_example_config_command(self):
    ...
    examples_path = Path(__file__).parent.parent.parent / "examples" / "cls_example"
    result = subprocess.run(
      [str(examples_path), "config-management", "set-default-mode", "--mode", "FAST"],
      capture_output=True,
      text=True,
      timeout=10
    )
