...
class Completion:
        ...
        if self._completion_handler:
            try:
                ...
                installer = CompletionInstaller(
                    self._completion_handler, prog_name, command_patterns
                )
            ...
            try:
                ...
                if "--_complete" in words:
                    ...
                    if complete_idx < len(sys.argv) - 1:
                        current_word = (
                            sys.argv[complete_idx + 1] if complete_idx + 1 < len(sys.argv) else ""
                        )
                ...
                parser = (
                    self._cli_instance.create_parser(no_color=True) if self._cli_instance else None
                )
    ...
    def is_completion_request(self) -> bool:
        ...
        return "--_complete" in sys.argv or os.environ.get("_FREYJA_COMPLETE") is not None
