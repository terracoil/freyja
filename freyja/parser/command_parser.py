"""Command parsing — builds argparse parsers from the discovered command tree."""
...
from ..theme import get_default_theme, get_no_color_theme
...
class CommandParser:
  ...
  def __init__(
    self, title: str, theme=None, alphabetize: bool = True, enable_completion: bool = False
  ):
  ...
  def create_parser(
    ...
    """
    # Use no-color theme when COLORS are disabled; otherwise use the configured theme.
    effective_theme = get_no_color_theme() if no_color else self.theme

    effective_alphabetize = self.alphabetize
    ...
    parser = argparse.ArgumentParser(
      description=enhanced_title, formatter_class=create_formatter_with_theme
    )

    # Add global arguments
    self._add_global_arguments(parser, target_class, command_tree)

    # Create subparsers for cmd_tree
    subparsers = parser.add_subparsers(
      title='COMMANDS', dest='command', required=False, help='Available cmd_tree', metavar=''
    )
  ...
  def _add_global_arguments(
    ...
    if target_class and any(info.get('type') == 'group' for info in command_tree.values()):
  ...
  def _add_flat_command_from_tree(
    ...
    def create_formatter_with_theme(*args, **kwargs):
      return HierarchicalHelpFormatter(*args, theme=theme, alphabetize=self.alphabetize, **kwargs)

    sub = subparsers.add_parser(
      command_name, help=desc, description=desc, formatter_class=create_formatter_with_theme
    )
  ...
  def _add_command_group_from_tree(
    ...
    group_parser = subparsers.add_parser(
      group_name, help=group_help, formatter_class=create_formatter_with_theme
    )
    ...
    if inner_class:
      ArgumentParser.add_subglobal_class_args(group_parser, inner_class, group_name)
  ...
  def _apply_parser_patches(self, parser: argparse.ArgumentParser, theme):
    """Apply title-styling patch to the top-level parser."""
    ...
    def patched_format_help():
      ...
      if self.title in original_help:
