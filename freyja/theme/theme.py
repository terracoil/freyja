...
class Theme:
    ...
    def create_adjusted_copy(
        self, adjust_percent: float, adjust_strategy: AdjustStrategy | None = None
    ) -> Theme:
        ...
        try:
            ...
            adjusted_top_level = CommandStyleSection(
                ...
                command_description=self.get_adjusted_style(
                    self.topLevelCommandSection.command_description
                ),
                option_name=self.get_adjusted_style(self.topLevelCommandSection.option_name),
                option_description=self.get_adjusted_style(
                    self.topLevelCommandSection.option_description
                ),
            )
            ...
            adjusted_command_group = CommandStyleSection(
                ...
                command_description=self.get_adjusted_style(
                    self.commandGroupSection.command_description
                ),
                option_name=self.get_adjusted_style(self.commandGroupSection.option_name),
                option_description=self.get_adjusted_style(
                    self.commandGroupSection.option_description
                ),
            )
            ...
            adjusted_grouped_command = CommandStyleSection(
                ...
                command_description=self.get_adjusted_style(
                    self.groupedCommandSection.command_description
                ),
                option_name=self.get_adjusted_style(self.groupedCommandSection.option_name),
                option_description=self.get_adjusted_style(
                    self.groupedCommandSection.option_description
                ),
            )
    ...
    def get_adjusted_style(self, original: ThemeStyle) -> ThemeStyle:
        ...
        if original.fg:
            ...
            adjusted_fg = original.fg.adjust(
                brightness=self.adjust_percent, strategy=self.adjust_strategy
            )
...
def create_default_theme() -> Theme:
    ...
    grouped_command_section = CommandStyleSection(
        command_name=ThemeStyle(bold=True),
        command_description=ThemeStyle(
            fg=RGB.from_rgb(ForeUniversal.BROWN.value), bold=True, italic=True
        ),
        option_name=ThemeStyle(),
