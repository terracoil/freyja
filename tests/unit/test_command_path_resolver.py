...
import pytest
from freyja.parser.command_path_resolver import CommandPath, CommandPathResolver
...
class MockCommandTree:
  ...
  def __init__(self):
    """Initialize mock command tree with empty commands and groups."""
    self.flat_commands = {}
    self.groups = {}
...
class TestCommandPathResolver:
  ...
  def test_extract_command_and_remaining(self, simple_tree):
    ...
    resolver = CommandPathResolver(simple_tree)
    command_path, remaining = resolver.extract_command_and_remaining(['test', '--option', 'value'])
