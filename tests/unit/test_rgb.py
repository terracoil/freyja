...
class TestRGBConversions:
  ...
  def test_to_ansi_foreground_256(self):
    """Force 256-color mode: foreground emits 38;5;NNN."""
    ansi = RGB.from_rgb(0xFF0000).to_ansi(background=False, truecolor=False)
    assert ansi.startswith('\033[38;5;')
    assert ansi.endswith('m')

  def test_to_ansi_background_256(self):
    """Force 256-color mode: background emits 48;5;NNN."""
    ansi = RGB.from_rgb(0x00FF00).to_ansi(background=True, truecolor=False)
    assert ansi.startswith('\033[48;5;')
    assert ansi.endswith('m')

  def test_to_ansi_foreground_truecolor(self):
    """Force truecolor: foreground emits 38;2;R;G;B."""
    ansi = RGB.from_rgb(0xCB4B16).to_ansi(background=False, truecolor=True)
    assert ansi == '\033[38;2;203;75;22m'

  def test_to_ansi_background_truecolor(self):
    """Force truecolor: background emits 48;2;R;G;B."""
    ansi = RGB.from_rgb(0xCB4B16).to_ansi(background=True, truecolor=True)
    assert ansi == '\033[48;2;203;75;22m'
