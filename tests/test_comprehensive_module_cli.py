...
def backup__create(source_dir: Path, destination: str, compress: bool = True) -> dict:
...
def backup__restore(backup_file: Path, target_dir: Path, 
...
def config__set_value(key: str, value: str, global_config: bool = False) -> dict:
...
def config__get_value(key: str, default_value: str = "none") -> dict:
...
def task__create(title: str, priority: Priority = Priority.MEDIUM,
...
def task__list_tasks(status: str = "all", priority_filter: Priority = Priority.MEDIUM,
...
class TestModuleCLI:
    ...
    def test_module_function_discovery(self):
        ...
        # Should have discovered public functions
        expected_functions = {
            'simple_function', 'process_data', 'analyze_logs',
            'backup__create', 'backup__restore',
            'config__set_value', 'config__get_value',
            'task__create', 'task__list_tasks',
            'export_data'
        }
    ...
    def test_hierarchical_command_execution(self):
        """Test execution of hierarchical commands."""
        cli = self.create_test_cli()
    ...
    def test_list_type_handling(self):
        ...
        test_args = ['task', 'create', '--title', 'Test Task', '--priority', 'HIGH']
        result = cli.run(test_args)
    ...
    def test_subcommand_help(self):
        ...
        # Test that we can parse help for subcommands without errors
...
class TestModuleCLIErrorHandling:
    ...
    def test_invalid_subcommand(self):
        ...
        with pytest.raises(SystemExit):
            cli.run(['backup', 'invalid-subcommand'])
...
class TestModuleCLICommandGrouping:
    ...
    def test_hierarchical_command_grouping(self):
        ...
        backup_subcommands = cli.commands['backup']['subcommands']
        assert 'create' in backup_subcommands
        assert 'restore' in backup_subcommands
        
        # Should create config command group
        assert 'config' in cli.commands
        config_subcommands = cli.commands['config']['subcommands']
        assert 'set-value' in config_subcommands
        assert 'get-value' in config_subcommands
        
        # Should create task command group
        assert 'task' in cli.commands
        task_subcommands = cli.commands['task']['subcommands']
        assert 'create' in task_subcommands
        assert 'list-tasks' in task_subcommands
    ...
    def test_mixed_flat_and_hierarchical_commands(self):
        ...
        # Flat commands should be type 'flat'
        assert cli.commands['simple-function']['type'] == 'flat'
        assert cli.commands['export-data']['type'] == 'flat'
        
        # Hierarchical commands should be type 'group'
        assert cli.commands['backup']['type'] == 'group'
        assert cli.commands['config']['type'] == 'group'
